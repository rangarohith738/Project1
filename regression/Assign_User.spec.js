...
test('Assign User in RFP @regression @set1', async ({ page }) => {
  ...
  const createRfpButton = page.getByRole('button', {name: 'Create Request for Proposal',exact: true});
  ...
  const statusDraftDiv = page.locator('div').filter({ hasText: 'Status Requested' }).first();
  ...
  const estimateDescriptionInput = page.locator('//textarea[@id="deliverableItem.description"]');
  await expect(estimateDescriptionInput).toBeVisible();
  await expect(estimateDescriptionInput).toBeEditable();
  await estimateDescriptionInput.fill(testData.estimate);
  ...
  const descriptionDiv = page.locator(`//div[@id="info-estimate"]//div[contains(normalize-space(),"${testData.estimate}")]`).first();
