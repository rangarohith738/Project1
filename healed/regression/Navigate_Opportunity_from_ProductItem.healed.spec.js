...
test('Navigate Opportunity from Product Item @regression @set2', async ({ page }) => {
  ...
  if (await page.locator('//button[contains(normalize-space(),"View All")]').first().isVisible()) {
    await heal(page, 'view all button', 'click', null,
      () => page.locator('//button[contains(normalize-space(),"View All")]').first());
  }
  ...
  await heal(page, 'open link', 'visible', null,
    () => page.locator('//div[normalize-space()="' + opportunityId + '"]//a').first());
  await heal(page, 'open link', 'click', null,
    () => page.locator('//div[normalize-space()="' + opportunityId + '"]//a').first());

  await page.waitForLoadState('domcontentloaded');
  await heal(page, 'opportunity page heading', 'visible', null,
    () => page.locator('//div[@id="info-project"][contains(normalize-space(),"' + opportunityId + '")]').first());
});
