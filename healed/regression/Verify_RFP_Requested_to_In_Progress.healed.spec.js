...
test('Verify RFP Requested to In Progress @regression @set1', async ({ page }) => {
  ...
  await heal(page, 'select user button', 'visible', null,
    () => page.locator('xpath=(//button[normalize-space()="Select User"])[1]'));
  ...
  await heal(page, 'rfp hyperlink', 'visible', null,
    () => page.locator('//div[normalize-space()="Request for Proposal"]/following::span[1]').first());
  const rfpHyperlinkText = (await page.locator('//div[normalize-space()="Request for Proposal"]/following::span[1]').first().innerText()).trim();
  console.log(`[data] RFP hyperlink: ${rfpHyperlinkText}`);
  await heal(page, 'rfp hyperlink', 'click', null,
    () => page.locator('//div[normalize-space()="Request for Proposal"]/following::span[1]').first());

  await page.waitForTimeout(2000);
  await heal(page, 'proposal title span back', 'visible', null,
    () => page.locator('span').filter({ hasText: `${rfpHyperlinkText} - Request for Proposal` }).first());

  await expect(page.locator('//span[normalize-space()="In Progress"]/preceding-sibling::div/span/span[normalize-space()="04"]')).toHaveCount(1);

  await heal(page, 'status in progress text', 'visible', null,
    () => page.locator('div').filter({ hasText: 'In Progress' }).first());
});
