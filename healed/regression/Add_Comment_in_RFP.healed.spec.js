...
test('Add Comment in RFP @regression @set1', async ({ page }) => {
  ...
  await heal(page, 'comments customer facing text', 'visible', null,
    () => page.locator('span').filter({ hasText: 'Comments (Customer facing)' }).first());

  await heal(page, 'new comment button', 'click', null,
    () => page.getByRole('button', { name: 'New Comment', exact: true }));

  await heal(page, 'comments textarea', 'fill', testData.description,
    () => page.locator('textarea[name="comments"]'));

  await page.locator('button[x-tooltip="Save"][wire\\:click="save"]').scrollIntoViewIfNeeded();
  await heal(page, 'save comment button', 'click', null,
    () => page.locator('button[x-tooltip="Save"][wire\\:click="save"]'));

  // 15. Confirm comment appears in the list (assert by text)
  await heal(page, 'comment text div', 'visible', null,
    () => page.locator('div').filter({ hasText: testData.description }).first());
});
