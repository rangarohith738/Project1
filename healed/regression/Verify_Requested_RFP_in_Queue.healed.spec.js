...
test('Verify Requested RFP in Queue @regression @set1', async ({ page }) => {
  ...
  await heal(page, 'rfp heading', 'visible', null,
    () => page.locator('//h1/span[contains(normalize-space(), "Request for Proposal")]'));
  const rfpText = await page.locator('//h1/span[contains(normalize-space(), "Request for Proposal")]').innerText();
  ...
  await heal(page, 'rfp cell customer name', 'visible', null,
    () => page.locator('td').filter({ hasText: testData.quickSearch }).first());
  await expect(page.locator('td').filter({ hasText: testData.quickSearch }).first()).toHaveText(testData.quickSearch);

  await heal(page, 'rfp cell plant', 'visible', null,
    () => page.locator('td').filter({ hasText: 'Fortis' }).first());
  await expect(page.locator('td').filter({ hasText: 'Fortis' }).first()).toHaveText('Fortis');

  await heal(page, 'rfp cell status', 'visible', null,
    () => page.locator('td').filter({ hasText: 'Requested' }).first());
  await expect(page.locator('td').filter({ hasText: 'Requested' }).first()).toHaveText('Requested');

  await heal(page, 'rfp cell created by', 'visible', null,
    () => page.locator('td').filter({ hasText: 'Ranga Sharan Rohith' }).first());
  await expect(page.locator('td').filter({ hasText: 'Ranga Sharan Rohith' }).first()).toHaveText('Ranga Sharan Rohith');

  await heal(page, 'product class cell', 'visible', null,
    () => page.locator('td').filter({ hasText: 'RFID Label' }).first());
  await expect(page.locator('td').filter({ hasText: 'RFID Label' }).first()).toHaveText('RFID Label');
});
