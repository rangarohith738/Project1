...
test('Verify Opportunity Navigation @regression @set1', async ({ page }) => {
  ...
  await heal(page, 'proposal title span', 'click', null,
    () => page.locator('span').filter({ hasText: `${rfpNumber} - Request for Proposal` }).first());

  await heal(page, 'opportunity link', 'visible', null,
    () => page.locator('//div[normalize-space()="Opportunity"]//parent::div//child::div/div'));
  await heal(page, 'opportunity link', 'click', null,
    () => page.locator('//div[normalize-space()="Opportunity"]//parent::div//child::div/div'));

  await page.waitForLoadState('domcontentloaded');
  await heal(page, 'opportunity div', 'visible', null,
    () => page.locator('div').filter({ hasText: /OPP\d+ - Opportunity/ }).first());
  await heal(page, 'request for proposal heading', 'visible', null,
